...
export const publish = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Compare against dailySum, not totals: both per-day arrays are truncated
    // by the HTTP layer to the same newest-N window, while totals stay
    // all-time. Comparing a truncated sum with all-time totals auto-flagged
    // every user whose history is longer than the window — exactly the heavy
    // users a launch board exists for.
    const dailyModelSpendDiverges =
      dailyModelSpend.length > 0 &&
      dailySum > 0 &&
      Math.abs(dailyModelSpendSum - dailySum) > dailySum * 0.02 + 1
    const suspicious =
